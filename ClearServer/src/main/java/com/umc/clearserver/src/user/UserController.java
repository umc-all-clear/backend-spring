...
public class UserController {
    ...
    public BaseResponse<PostSignUpRes> createUser(@RequestBody PostSignUpReq postSignUpReq) {
        ...
        // 비밀번호 관련!
        if (postSignUpReq.getPassword1() == null) {
            return new BaseResponse<>(POST_USERS_EMPTY_PASSWORD);
        }
        if (postSignUpReq.getPassword2() == null){
            return new BaseResponse<>(POST_USERS_EMPTY_PASSWORD);
        }
...
//        if (!isRegexPassword(postSignUpReq.getPassword1()))
//        {
//            return new BaseResponse<>(POST_USERS_INVALID_PASSWORD);
//        }
        // 닉네임 관련
    ...
    public static boolean isRegexPassword(String password)
    {
        ...
        {
            for(int j=0; j< specialKeyArr.length; j++)
            {
                if(password.charAt(i)==specialKeyArr[j])
                {
